...
public class LockscreenStyle extends SettingsPreferenceFragment
    ...
    private void resizeioapLock() {
        Bitmap ioapLock = BitmapFactory.decodeResource(getResources(), R.drawable.ioap_lock);
        if (ioapLock != null) {
            String path = null;
            int px = requestImageSize();
            ioapLock = Bitmap.createScaledBitmap(ioapLock, px, px, true);
            try {
                mLockImage.createNewFile();
                mLockImage.setWritable(true, false);
                File image = new File(getActivity().getFilesDir() + File.separator
                ...
                ioapLock.compress(Bitmap.CompressFormat.PNG, 100, outPut);
                image.setReadable(true, false);
                outPut.flush();
                outPut.close();
            } catch (Exception e) {
